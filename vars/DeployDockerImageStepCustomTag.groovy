/**@
    ...
    parameters :
        ...
        customTag     : Custom tag for Docker image (e.g., v3.0.0)
...
def call(Map config = [:]) {
    if (!config.customTag) {
        error "customTag parameter is required for DeployDockerImageStepCustomTag"
    }
    config.imageName = config.imageName.replaceAll("/null/", "/").replaceAll("//", "/").replaceFirst(":/+", "://")
    def buildArgs = config.baseHref ? "--build-arg baseHref=${config.baseHref} ." : "."
