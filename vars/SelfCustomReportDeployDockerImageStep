...
def call(Map config = [:]) {
    ...
    if (config.cloudType == "Alibaba Cloud") {
        docker.withRegistry("${config.registryURL}", "${config.credentialsId}") {
            dockerImageRemote = docker.build("${config.imageName}:build-${env.BUILD_ID}", "--build-arg IMAGE_NAME=${config.sourceImageName} --build-arg IMAGE_TAG=${config.sourceImageTag} .")
