...
def call(Map config = [:]) {
    ...
    if (config.cloudType == "Alibaba Cloud") {
        ...
        sh "export VAULT_ADDR=https://omnisecret.bfidigital.id"
        ...
        OIDC_TOKEN=$(curl -s \
          ...
          "$VAULT_ADDR/v1/identity/oidc/token/role-001" \
          | jq -r '.data.token')
          '''
        ...
        sh ''' echo "$FEDERATED_TOKEN" | docker login -u oauth2accesstoken --password-stdin asia-southeast2-docker.pkg.dev '''
        dockerImageRemote = docker.build "${config.imageName}:build-${env.BUILD_ID}"
        dockerImageRemote.push()
        dockerImageRemote.push("cloud")
    }
